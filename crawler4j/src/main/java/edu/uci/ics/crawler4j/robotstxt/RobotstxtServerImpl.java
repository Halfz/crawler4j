 ...
 */
public class RobotstxtServerImpl implements RobotstxtServer {

    private static final Logger logger = LoggerFactory.getLogger(RobotstxtServerImpl.class);
    protected final Map<String, HostDirectives> host2directivesCache = new HashMap<>();
    private final int maxBytes;
    protected RobotstxtConfig config;
    protected PageFetcher pageFetcher;

    public RobotstxtServerImpl(RobotstxtConfig config, PageFetcher pageFetcher) {
        this(config, pageFetcher, 16384);
    }

    public RobotstxtServerImpl(RobotstxtConfig config, PageFetcher pageFetcher, int maxBytes) {
        this.config = config;
    ...
    private HostDirectives fetchDirectives(URL url) {
        ...
        try {
            for (int redir = 0; redir < 3; ++redir) {
                fetchResult = pageFetcher.fetchPage(robotsTxtUrl).toCompletableFuture().get();
                int status = fetchResult.getStatusCode();
                // Follow redirects up to 3 levels
                if ((status == HttpStatus.SC_MULTIPLE_CHOICES || status == HttpStatus.SC_MOVED_PERMANENTLY || status == HttpStatus.SC_MOVED_TEMPORARILY || status == HttpStatus.SC_SEE_OTHER || status == HttpStatus.SC_TEMPORARY_REDIRECT || status == 308) &&
            ...
            if (fetchResult.getStatusCode() == HttpStatus.SC_OK) {
                ...
                // https://goo.gl/OqpKbP
                page.applyPageFetchResult(fetchResult, 10_000 * 1024);
