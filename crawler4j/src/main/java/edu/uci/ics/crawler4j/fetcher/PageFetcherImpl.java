...
public class PageFetcherImpl implements PageFetcher {
    protected static final Logger logger = LoggerFactory.getLogger(PageFetcherImpl.class);
    ...
    public PageFetcherImpl(CrawlConfig config) {
        this.config = config;

        RequestConfig requestConfig =
                RequestConfig.custom().setExpectContinueEnabled(false).setCookieSpec(config.getCookiePolicy()).setRedirectsEnabled(false).setSocketTimeout(config.getSocketTimeout()).setConnectTimeout(config.getConnectionTimeout()).build();
        ...
        if (authInfos != null) {
            ...
            authInfos.stream().filter(info -> AuthInfo.AuthenticationType.FORM_AUTHENTICATION.equals(info.getAuthenticationType())).map(FormAuthInfo.class::cast).forEach(this::doFormLogin);
        } else {
            httpClient = clientBuilder.build();
        }
    ...
    public CompletionStage<PageFetchResult> fetchPage(WebURL webUrl) {
        ...
        if (config.getPolitenessDelay() > 0) {
            ...
            synchronized (mutex) {
                ...
                if ((now - lastFetchTime) < config.getPolitenessDelay()) {
                    sleep = config.getPolitenessDelay() - (now - lastFetchTime);
                }
        ...
        return CompletableFuture.completedFuture(fetchPageSync(webUrl));
    }
    ...
    protected PageFetchResult fetchPageSync(WebURL webUrl) {
        ...
        try {
            ...
            // If Redirect ( 3xx )
            if (statusCode == HttpStatus.SC_MOVED_PERMANENTLY || statusCode == HttpStatus.SC_MOVED_TEMPORARILY || statusCode == HttpStatus.SC_MULTIPLE_CHOICES || statusCode == HttpStatus.SC_SEE_OTHER || statusCode == HttpStatus.SC_TEMPORARY_REDIRECT || statusCode == 308) { // todo follow
                ...
                if (fetchResult.getEntity() != null) {
                    ...
                    if (size > config.getMaxDownloadSize()) {
                        ...
                        response.close();
                        throw new CompletionException(new PageBiggerThanMaxSizeException(size));
                    }
                }
            }
    ...
    private void addNtCredentials(NtAuthInfo authInfo, Map<AuthScope, Credentials> credentialsMap) {
        ...
        try {
            Credentials credentials = new NTCredentials(authInfo.getUsername(), authInfo.getPassword(),
